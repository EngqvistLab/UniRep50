...
class mLSTMCell1900(tf.compat.v1.nn.rnn_cell.RNNCell):
    ...
    def call(self, inputs, state):
        ...
        nin = inputs.get_shape()[1]

        # Unpack the state tuple
        c_prev, h_prev = state
        with tf.compat.v1.variable_scope(self._scope, reuse=tf.compat.v1.AUTO_REUSE) as scope:
...
class mLSTMCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    ...
    def __init__(self,
                 num_units,
                 wx_init=tf.compat.v1.keras.initializers.Orthogonal(),
                 wh_init=tf.compat.v1.keras.initializers.Orthogonal(),
                 wmx_init=tf.compat.v1.keras.initializers.Orthogonal(),
                 wmh_init=tf.compat.v1.keras.initializers.Orthogonal(),
                 b_init=tf.compat.v1.keras.initializers.Orthogonal(),
                 gx_init=tf.compat.v1.keras.initializers.Ones,
                 gh_init=tf.compat.v1.keras.initializers.Ones,
                 gmx_init=tf.compat.v1.keras.initializers.Ones,
                 gmh_init=tf.compat.v1.keras.initializers.Ones,
                 wn=True,
                 scope='mlstm',
                 var_device='cpu:0',
                 ):
    ...
    def call(self, inputs, state):
        ...
        # Unpack the state tuple
        c_prev, h_prev = state
        with tf.compat.v1.variable_scope(self._scope) as scope:
            # scope.reuse_variables()
            wx = tf.compat.v1.get_variable(
...
class babbler1900():
    ...
    def __init__(self, model_path=resource_filename(__name__, "1900_weights"), batch_size=256):
        self._rnn_size = 1900
        self._vocab_size = 26
        self._embed_dim = 10
        self._wn = True
        ...
        self._minibatch_x_placeholder = tf.compat.v1.placeholder(tf.int32, shape=[None, None], name="minibatch_x")
        self._minibatch_y_placeholder = tf.compat.v1.placeholder(tf.int32, shape=[None, None], name="minibatch_y")
        ...
        self._seq_length_placeholder = tf.compat.v1.placeholder(tf.int32, shape=[None], name="seq_len")
        ...
        # get sequence embeddings at input layer
        embed_matrix = tf.compat.v1.get_variable("embed_matrix", dtype=tf.float32, initializer=np.load(os.path.join(self._model_path, "embed_matrix:0.npy")))
        embed_cell = tf.compat.v1.nn.embedding_lookup(embed_matrix, self._minibatch_x_placeholder)

        # self._output, self._final_state = keras.layers.RNN(rnn)(inputs=embed_cell, initial_state=self._initial_state_placeholder)

        self._output, self._final_state = tf.compat.v1.nn.dynamic_rnn(
            rnn,
            embed_cell,
        ...
        logits_flat = tf.compat.v1.keras.layers.Dense(
            units=self._vocab_size - 1, activation=None, input_shape=flat.shape,
            kernel_initializer=tf.compat.v1.constant_initializer(np.load(os.path.join(self._model_path, "fully_connected_weights:0.npy"))),
            bias_initializer=tf.compat.v1.constant_initializer(np.load(os.path.join(self._model_path, "fully_connected_biases:0.npy")))).output_shape

        self._logits = tf.reshape(logits_flat, [batch_size, tf_get_shape(self._minibatch_x_placeholder)[1], self._vocab_size - 1])
        ...
        self._loss = tf.compat.v1.reduce_mean(batch_losses)
    ...
    def dump_weights(self, sess, dir_name=resource_filename(__name__, "1900_weights")):
        """
